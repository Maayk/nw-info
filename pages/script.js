document.addEventListener("DOMContentLoaded", () => {
    ...
    function movePlayerToSlot(playerName, sourceElement, targetSlot) {
        ...
        const playerColor = playerColorMap[playerName] || "transparent";
    ...
    function loadGroupData() {
        ...
        Object.keys(groupData).forEach(playerName => {
            ...
            loadedNames.add(playerName); 
        });
    

        const allNames = nameTextarea.value.split('\n').map(name => name.trim()).filter(name => name);
    ...
    window.removePlayer = function(playerName) {
        ...
        if (slot) {
            ...
            playerElement.draggable = true;
            playerElement.id = `name-${formatNameToId(playerName)}`;
            playerElement.style.backgroundColor = playerColorMap[playerName] || "transparent";
            playerElement.addEventListener("dragstart", handleDragStart);
            playerElement.addEventListener("contextmenu", (e) => {
                e.preventDefault();
                showColorMenu(e, playerName);
            });
            playerListContainer.appendChild(playerElement);
